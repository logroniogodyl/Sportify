	...
	if (idSoc == "0") {
		...
		function handleResponsePrenServlet(response) {
			if (response == "errore") {
				...
				document.getElementById("redirectPren").innerHTML = "Prenotazione a nome di " + nome + "<br> effettuata. <br> Controlla la tua email per i dettagli della prenotazione!<br><button onclick=\"remOverlay()\" style=\'height:30px; background-color: #225FE3; color: white; border-radius: 10px; margin-top: 10px;\'>SIUM</button>";
				checkPren()
			}
		}
	}
		...
		function handleResponsePrenServlet(response) {
			if (response == "errore") {
				...
				formPren.style.display = "none";
				document.getElementById("redirectPren").style.display = "block"
				document.getElementById("redirectPren").innerHTML = "Prenotazione a nome di " + nome + "<br> effettuata. <br> Controlla la tua email per i dettagli della prenotazione!<br><button onclick=\"remOverlay()\" style=\'height:30px; background-color: #225FE3; color: white; border-radius: 10px; margin-top: 10px;\'>SIUM</button>";
				checkPren()
			}
		}
